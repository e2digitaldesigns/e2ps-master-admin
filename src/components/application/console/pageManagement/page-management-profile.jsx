import React, { useEffect, useState } from 'react';
import { useDispatch } from 'react-redux';
import { toast } from 'react-toastify';
import { fetchPageProfile } from '../../../../redux/actions/pages/pageProfileActions';
// import ErrorPage from '../_utils/_errors/error';
import LoadingPage from '../_utils/_loading/loading';
import PageProfileMain from '../pageManagement/page-management-profile-main';

export default (props) => {
  const dispatch = useDispatch();
  // const state = useSelector((state) => state.pageProfile);
  const [documentState, setDocumentState] = useState({ docReady: false });

  useEffect(() => {
    const loadData = async () => {
      try {
        const result = await dispatch(fetchPageProfile(props.match.params.id));

        if (result.error.errorCode === '0x0') {
          setDocumentState((documentState) => ({
            ...documentState,
            docReady: true,
          }));
        } else {
          throw result;
        }
      } catch (err) {
        toast.error(err.error.errorDesc);
      }
    };

    loadData();
  }, [dispatch, props.match.params.id]);

  // if (state.error !== null) return <ErrorPage error={state.error} />;

  if (!documentState.docReady) return <LoadingPage />;

  return <PageProfileMain />;
};
