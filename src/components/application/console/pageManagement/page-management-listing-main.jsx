...
export default () => {
  ...
  const staff = useSelector((state) => state.pageListing.dataSet);
  ...
  const linkParser = (page, pageSize, filter) => {
    console.clear();
    const linker = `/console/page-management/listing/?page=${page}&results=${pageSize}&st=${filter}`;
    history.push(linker);
  };
            ...
            {listing.map((m) => (
              <tr key={m._id} className="listingTable-tr">
                <td>
                  {m.name}
                  <br />
                  <Link to={'/console/page-management/profile/' + m._id}>
                    Edit Page
                  </Link>
                </td>
